...
let ava = []; // i je i board takze od 0
...
function gameClick(el) {
  ...
  ava.splice(ava.indexOf([x - 1, y - 1]), 1);
  board[x - 1][y - 1] = cp;
  cp = p2;
  loop();
  if (mode == 1) round();
...
function setField(x, y, type, elem) {
  ...
  if (type == 0) {
    el.src = document.location.origin + "/O.png";
    return;
  } else if (type == 1) {
    el.src = document.location.origin + "/X.png";
    return;
  } else if (type == null) {
    el.src = document.location.origin + "/blank.png";
    return;
  } else if (type == 2) {
    el.src = document.location.origin + "/O-colored.png";
    return;
  } else if (type == 3) {
    el.src = document.location.origin + "/X-colored.png";
    return;
  }
}
