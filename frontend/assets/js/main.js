...
function checkForWinner() {
  ...
  for(let i = 0; i < 3; i++) {
    if (board[i][0] == board[i][1] && board[i][1] == board[i][2]) {
      if (board[i][0] != null) {
        winner = board[i][0];
        winningSection = [[i+1,1],[i+1,2],[i+1,3]];
      }
    }
  }
  // svisle
  for(let i = 0; i < 3; i++) {
    if (board[0][i] == board[1][i] && board[1][i] == board[2][i]) {
      if (board[0][i] != null) {
        winner = board[0][i];
        winningSection = [[1,i+1],[2,i+1],[3,i+1]];
      }
    }
  }
  // diagonalne
  if (board[0][0] == board[1][1] && board[1][1] == board[2][2]){
    if (board[0][0] != null) {
      winner = board[0][0];
      winningSection = [[1,1],[2,2],[3,3]];
    }
  }
  if (board[0][2] == board[1][1] && board[1][1] == board[2][0]){
    if (board[2][0] != null) {
      winner = board[2][0];
      winningSection = [[1,3],[2,2],[3,1]];
    }
  }
...
function restart(){
  ...
  invis(true, ['game-row-1', 'game-row-2', 'game-row-3'], ['game-btn-human', 'game-btn-ai']);
...
function startButton(el) {
  ...
  if (el.id == 'game-btn-ai') {
    loop();
    mode = 1;
    invis(false, ['game-row-1', 'game-row-2', 'game-row-3', 'game-round', 'game-btn-restart'], ['game-btn-human', 'game-btn-ai']);
...
function minimaxCFEFS() { // call minimax for every free spot
...
function difficultySet(el) {
    if (el.id == "game-btn-dif-easy" || el.id == "game-btn-dif-medium" || el.id == "game-btn-dif-hard") {
        invis(true, [], ["game-btn-dif-easy", "game-btn-dif-medium", "game-btn-dif-hard"]);
